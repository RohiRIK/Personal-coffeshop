...
export function OrderDetailsModal({ order, onClose }: OrderDetailsModalProps) {
  const { insights, loading: insightsLoading } = useCustomerInsights(
    order?.userId,
  );
      ...
      >
          ...
          <div className="flex justify-between items-start p-6 border-b border-stone-800 bg-stone-900/50">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getStatusColor(order.status)} uppercase tracking-wider`}
                >
                  {order.status}
                </span>
              </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-stone-500 mt-1 flex flex-wrap gap-x-3 gap-y-1">
                        {item.milk && (
                          <span>
                            Milk:{" "}
                            <span className="text-stone-400">{item.milk}</span>
                          </span>
                        )}
                        {item.cup && (
                          <span>
                            Cup:{" "}
                            <span className="text-stone-400">{item.cup}</span>
                          </span>
                        )}
                      </div>
        ...
        <div className="w-full md:w-80 border-t md:border-t-0 md:border-l border-stone-800 bg-stone-950 p-6 flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <div className="font-bold text-stone-200 text-lg">
          ...
          {insightsLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="text-xs text-stone-500 mb-2 font-bold uppercase">
                  Preferences
                </div>
                <div className="space-y-3">
                  ...
                  {insights.favoriteMilk &&
                    insights.favoriteMilk !== "None" && (
                      <div className="flex items-center gap-3 bg-stone-900/50 p-3 rounded-lg">
