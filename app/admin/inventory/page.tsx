...
export default function InventoryPage() {
    const { isAvailable, toggleAvailability, loading } = useInventory();
    ...
    return (
        <div>
            ...
            <p className="text-stone-400 mb-8">Manage real-time availability of ingredients. Changes update instantly on the menu.</p>
                    ...
                    <div className="space-y-4">
                        {MILK_OPTIONS.map((item) => (
                            <InventoryItemRow
                                ...
                                onToggle={() => toggleAvailability(item.id, isAvailable(item.id))}
                            />
                        ))}
                    </div>
                </section>
...
function InventoryItemRow({ name, available, onToggle }: { name: string, available: boolean, onToggle: () => void }) {
    return (
        <div className="flex items-center justify-between p-4 bg-stone-800 rounded-xl border border-stone-700">
            <span className={`font-medium ${available ? 'text-stone-200' : 'text-stone-500 line-through'}`}>
                {name}
            </span>
