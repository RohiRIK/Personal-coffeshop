    ...
    if (!authLoading && items.length === 0) {
        return (
            <div className="min-h-screen bg-stone-900 flex flex-col items-center justify-center p-4">
                <h1 className="text-3xl font-bold text-stone-100 mb-4">Your cart is empty</h1>
                <Link href="/menu" className="px-6 py-3 bg-amber-500 text-stone-900 font-bold rounded-xl hover:bg-amber-400 transition-colors">
                    Browse Menu
                </Link>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-stone-900 py-12 px-4">
            <div className="max-w-3xl mx-auto">
                <Link href="/menu" className="text-amber-400 hover:text-amber-300 text-sm mb-6 inline-block">
                    ← Back to Menu
                </Link>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div className="bg-stone-800 rounded-2xl p-6 border border-stone-700">
                            <h2 className="text-xl font-bold text-stone-100 mb-4">Your Items</h2>
                            <div className="space-y-4">
                                {items.map((item) => (
                                    <div key={item.id} className="flex gap-4 p-3 bg-stone-900/50 rounded-xl">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium text-amber-400">${(item.price * item.quantity).toFixed(2)}</span>
                                            </div>
                                            ...
                                            <div className="text-xs text-stone-500 mt-1">
                                                ...
                                                {item.options.instructions && <div className="italic mt-0.5">"{item.options.instructions}"</div>}
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="bg-stone-800 rounded-2xl p-6 border border-stone-700">
                            <h2 className="text-xl font-bold text-stone-100 mb-4">User Details</h2>
                            <div className="space-y-2 text-stone-300">
                                <p><span className="text-stone-500">Name:</span> {user?.displayName || 'Guest'}</p>
                    ...
                    <div className="md:col-span-1">
                        <div className="bg-stone-800 rounded-2xl p-6 border border-stone-700 sticky top-24">
                            ...
                            <div className="mb-6">
                                <label className="block text-sm font-medium text-stone-300 mb-2">Payment Method</label>
