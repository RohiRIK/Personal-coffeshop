...
export default function CustomersPage() {
  ...
  if (loading)
    return <div className="text-stone-400">Loading analytics...</div>;
  ...
  return (
    <div>
      ...
      <p className="text-stone-400 mb-8">
        Top performing customers and lifetime value.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <StatCard
          ...
          value={`$${(customers.reduce((sum, c) => sum + c.totalSpent, 0) / customers.reduce((sum, c) => sum + c.orderCount, 0) || 0).toFixed(2)}`}
        />
      </div>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-stone-900 p-6 rounded-2xl border border-stone-800">
      <h3 className="text-stone-500 text-sm font-medium mb-1 uppercase tracking-wider">
        {label}
      </h3>
