...
import { Loader2, Package, Clock, CheckCircle2, Coffee, ArrowRight } from 'lucide-react';
        ...
        <div className="min-h-screen bg-stone-950 text-stone-100 pb-20 pt-24 px-4">
            <div className="max-w-2xl mx-auto space-y-8">
                ...
                <header>
                    ...
                    <p className="text-stone-400">Track current orders and view your history.</p>
                </header>
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            {activeOrders.map(order => (
                                <div key={order.id} className="bg-stone-900 rounded-2xl p-6 border border-amber-500/20 shadow-lg shadow-amber-900/10 relative overflow-hidden">
                                    <div className="absolute top-0 right-0 p-4">
                                        <StatusBadge status={order.status} size="large" />
                                    </div>

                                    <div className="mb-4">
                                        <div className="text-xs text-stone-500 uppercase tracking-wider mb-1">Order #{order.id.slice(-4)}</div>
                                        ...
                                        {order.items.map((item, idx) => (
                                            <div key={idx} className="flex justify-between items-start text-sm">
                                                <div className="flex gap-3">
                                                    ...
                                                    <div>
                                                        <span className="text-stone-200 block">{item.name}</span>
                ...
                <section>
                    ...
                    <div className="space-y-3">
                        {pastOrders.map(order => (
                            <div key={order.id} className="bg-stone-900/50 hover:bg-stone-900 transition-colors rounded-xl p-4 border border-stone-800 flex items-center justify-between group">
                        ...
                        {orders.length === 0 && (
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-lg font-bold text-stone-300 mb-2">No orders yet</h3>
                                <p className="text-stone-500 mb-6 max-w-xs mx-auto">Looks like you haven't tried our delicious coffee yet!</p>
                                <Link
                                    href="/menu"
